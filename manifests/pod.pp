define xcat::pod(
  ...
  if $private_hash == undef or $private_hash['master_if'] == undef {
    fail "xcat::pod ${name} requires \$master_if to be defined in \$private_hash"
  }
  if $private_hash['master_ip'] == undef {
    fail "xcat::pod ${name} requires \$master_ip to be defined in \$private_hash"
  }
  if $private_hash['master_mac'] == undef {
    fail "xcat::pod ${name} requires \$master_mac to be defined in \$private_hash"
  }
  ...
  if $ipmi_hash != undef and $ipmi_hash['master_if'] != undef {
    if $ipmi_hash['master_ip'] == undef {
      fail "xcat::pod ${name} requires \$master_ip to be defined in \$ipmi_hash"
    }
    if $ipmi_hash['master_mac'] == undef {
      fail "xcat::pod ${name} requires \$master_mac to be defined in \$ipmi_hash"
    }
